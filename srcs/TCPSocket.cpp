...
/**
 * @brief               Read data from a given socket (connection already accepted)
 * 
...
void TCPSocket::startConnection() {
	...
	if (bind(passiveSocket, (sockaddr *)&socket_address, socket_address_len) < 0) {
		log("Cannot connect socket to address");
		return ;
	}
	// fcntl(passiveSocket, F_SETFL, O_NONBLOCK, FD_CLOEXEC);
...
/**
 * @brief               Accept an incoming connection to the listening server socket
 * 
 * @return int 
 */
int TCPSocket::acceptConnection() {
    int peerSocket = accept(passiveSocket, (sockaddr *)&socket_address,
						&socket_address_len);
	if (peerSocket < 0) {
		std::ostringstream ss;
	...
	// fcntl(peerSocket, F_SETFL, O_NONBLOCK, FD_CLOEXEC);
 ...
 */
void TCPSocket::closeConnection(int peerSocket) {
	close(peerSocket);
}
 ...
 * @return const int& 
 */
const int &TCPSocket::getPassiveSocket() {
	return (passiveSocket);
}

/**
 * @brief               overridden what method for the server socket exception class
 * 
 * @return const char* 
 */
